...
export const PROPOSAL_CATALOG: ProposalTypeEntry[] = [
  ...
  },
  {
    ...
    effect: "Fund the validators' monthly-withdraw credit line (tokens + monthly limits)",
    target: "GovPool.setCreditInfo(tokens, amounts)",
    needsIpfs: true,
    gating: ["validator"],
    mcpTool: "dexe_proposal_create (proposalType: validators_allocation)",
    implemented: true,
  },
  {
  ...
  // ---------- Internal (GovValidators.createInternalProposal) ----------
  {
    ...
    effect: "Directly change validator stake amounts (type 1). Set balance=0 to remove.",
    target: "GovValidators.createInternalProposal(type=1, url, data) — data=changeBalances(balances,users)",
    needsIpfs: true,
    gating: ["validator"],
  ...
  },
  {
    ...
    effect: "Change validator voting duration / executionDelay / quorum (type 0)",
    target: "GovValidators.createInternalProposal(type=0, url, data) — data=changeSettings(duration,executionDelay,quorum)",
    needsIpfs: true,
    gating: ["validator"],
