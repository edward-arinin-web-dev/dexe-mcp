...
| `dexe_agents_list` | The agent keyring (v0.28): every keyring signer (`DEXE_AGENT_PK_*` or the `AGENT_PK_*` alias; `AGENT_FUNDER_PK` → `funder`) with its `signerKey` ('agent1'…, 'funder'), address, native balance, optional ERC20 balance. Addresses only — keys never leave the server. | `DEXE_AGENT_PK_*`, `DEXE_RPC_URL` |
| `dexe_agents_fund` | Top up keyring wallets — from the PRIMARY signer by default, or `source:'funder'` to send from the `AGENT_FUNDER_PK` wallet (native or ERC20). Guards: recipients can only be keyring addresses; per-agent amount capped by `DEXE_AGENT_FUND_MAX_WEI` (default 0.1 native); top-up semantics (sends only the shortfall). | `DEXE_PRIVATE_KEY`, `DEXE_AGENT_PK_*`, `DEXE_RPC_URL` |
