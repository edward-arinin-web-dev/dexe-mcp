...
| `DEXE_MIN_SAFE_QUORUM_PCT` | treasury-drain guard (Q-1) | Minimum safe quorum percent (0–100). DAOs whose quorum is below this are flagged; `dexe_proposal_vote_and_execute` refuses to auto-execute a below-floor treasury proposal unless `acknowledgeRisk:true`. Default `50` (advisory recommends 51%+). | `50`, `51`, `66` |
| `DEXE_TREASURY_GUARD` | treasury-drain guard (Q-1) | Guard posture: `off` \| `warn` \| `refuse`. `warn` (default) = advisories everywhere + execute refusal; `refuse` = also hard-refuses below-floor `dexe_dao_build_deploy`; `off` = disabled. | `warn` |
