...
| `DEXE_TOOLSETS` | tool gating (`tools/list` size) | Comma list of tool profiles to register: `core`, `proposals`, `read`, `vote`, `governor`, `dev`, or `full`. **Default `core`** since v0.31.0 (43 of 167 tools, ~82 KB of `tools/list`) — it carries the composites plus the zero-config reporting reads; the ~30 single-purpose `dexe_proposal_build_*` tools moved to `proposals`, since `dexe_proposal_create` already covers every catalog type. `DEXE_TOOLSETS=core,proposals` restores the pre-0.31 surface. `full` loads everything; an **unknown name is dropped** (the recognized sets still apply) rather than escalating to `full`. `dexe_doctor` reports the active set; restart Claude Code after editing. See [TOOLS.md § Toolset profiles](./TOOLS.md#toolset-profiles). | `core,proposals`, `full`, `core,proposals,vote,read` |
